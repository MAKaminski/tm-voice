...
| `TMOS_SUPABASE_URL`, `TMOS_SERVICE_KEY` | Supabase → project `uzvbzusomftegypxudbj` ("TM1") → Settings → API → **service_role** | The **same project** as tm-voice's `DATABASE_URL`, reached a different way: `ops` over PostgREST, `agents` over Drizzle. Rule 4 still forbids reaching `ops` through Drizzle. service_role bypasses RLS and writes every table in the project — including `agents` — so set it on the **worker only**; capture never needs it. Requires the one-time `ops.tasks.external_key` DDL first (`docs/RUNBOOK.md` § 6b.2). |
