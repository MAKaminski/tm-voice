...
import { DLQ_NAME, QUEUES, type QueueName, getConfig } from "@tm/shared";
...
const dead = (await dlq.getJob(`dead:${queueName}:${jobId}`)) ?? (await dlq.getJob(jobId));
...
const j = await target.add(_origin.name, data, { jobId: `${String(data.idempotency_key)}:replay:${Date.now()}` });
await dead.remove();
